...
const (
	...
	RunIDEnv = "GOMCDC_RUN_ID"

	packageName          = "runtimecov"
	directoryPrefix      = "gomcdc_runtime_"
	eventFilePrefix      = "gomcdc-events-v1-"
	diagnosticFilePrefix = "gomcdc-diagnostics-v1-"
...
func Inject(ctx context.Context, moduleDir, modulePath string) (_ Package, err error) {
	return injectRuntimeSource(ctx, moduleDir, modulePath, runtimeSource)
}

// injectRuntimeSource is the single writer-source boundary. Production uses
// runtimeSource; tests may pass an instrumented variant without adding
// accounting branches to the generated hot path.
func injectRuntimeSource(ctx context.Context, moduleDir, modulePath, source string) (_ Package, err error) {
...
func CollectDetailed(ctx context.Context, dataDir string) (RecordedEvidence, error) {
	...
	for _, entry := range entries {
		...
		if validCompactionArtifactName(entry.Name()) {
			collector.interruption(path, 0, false, "incomplete event journal compaction")
			continue
		}
...
func (collector *eventCollector) consume(path string, line int, record wireRecord) {
	...
	case "clause":
		...
		collector.addClause(RecordedClauseEvent{
			...
			Event:            event,
			AlternativeIndex: optionalUint16(record.AlternativeIndex),
			AlternativeKnown: record.AlternativeIndex != nil,
		})
	case "diagnostic":
...
func (collector *eventCollector) sort() {
	...
	sort.Slice(collector.result.ClauseEvents, func(i, j int) bool {
		...
		if left.AlternativeKnown != right.AlternativeKnown {
			return !left.AlternativeKnown
		}
...
func validCompactionArtifactName(name string) bool {
	...
	journalName, nonce, found := strings.Cut(strings.TrimPrefix(name, "."), ".compact-")
	return found && nonce != "" && strings.HasPrefix(journalName, eventFilePrefix) &&
		strings.HasSuffix(journalName, eventFileSuffix)
}

const runtimeSource = `// Code generated by gomcdc. DO NOT EDIT.
...
const (
	...
	// A checkpoint rewrites no more often than each MiB of appended journal.
	// Once the unique snapshot grows, require twice its size in new tail data
	// so repeated checkpoints do not dominate writes.
	minimumCompactTailBytes = int64(1 << 20)
	compactSnapshotRatio = int64(2)
)
...
func (hooks Hooks) SelectClause(clauseID uint64, switchIDs ...uint64) {
	...
	recordClause(hooks.packagePath, switchID, clauseID, nil, "body-execution")
}

func (hooks Hooks) NoMatch(switchID uint64) {
	defer func() { _ = recover() }()
	recordClause(hooks.packagePath, switchID, 0, nil, "no-match-selection")
}

//go:noinline
func (hooks Hooks) CompilerDirectClause(clauseID, switchID, alternativeIndex uint64) {
	defer func() { _ = recover() }()
	var alternative *uint16
	if alternativeIndex <= uint64(^uint16(0)) {
		value := uint16(alternativeIndex)
		alternative = &value
	}
	recordClause(hooks.packagePath, switchID, clauseID, alternative, "direct-selection")
}

//go:noinline
func (hooks Hooks) CompilerNoMatch(switchID uint64) {
	defer func() { _ = recover() }()
	recordClause(hooks.packagePath, switchID, 0, nil, "no-match-selection")
}
...
func writeTerminal(item *evaluation, event record) {
	...
	state.mu.Lock()
	if writeEventLocked(state, event) {
		activeMu.Lock()
		if current := active[event.EvaluationID]; current == item { delete(active, event.EvaluationID) }
		activeMu.Unlock()
...
func recordClause(packagePath string, switchID, clauseID uint64, alternativeIndex *uint16, event string) {
	state := writerFor(packagePath)
	if state == nil { return }
	state.mu.Lock()
	defer state.mu.Unlock()
	alternative := "none"
	if alternativeIndex != nil { alternative = strconv.FormatUint(uint64(*alternativeIndex), 10) }
	key := packagePath + "\x00" + strconv.FormatUint(switchID, 10) + "\x00" + strconv.FormatUint(clauseID, 10) + "\x00" + alternative + "\x00" + event
	if _, exists := state.seenClauses[key]; exists { return }
	if writeEventLocked(state, record{Type:"clause", SwitchID: switchID, ClauseID:clauseID, AlternativeIndex:alternativeIndex, Event:event}) {
...
func writerFor(packagePath string) *writerState {
	...
	state = &writerState{
		...
		path: created.Name(),
		file: created,
		compactAtTailBytes: minimumCompactTailBytes,
...
func writeEventLocked(state *writerState, event record) bool {
	if event.Type == "begin" {
		if _, recorded := state.beginIDs[event.EvaluationID]; recorded { return true }
	}
	if state.file == nil {
		reopened, err := os.OpenFile(state.path, os.O_WRONLY|os.O_APPEND, 0)
		if err != nil { diagnostic(state.packagePath, "reopen event file: " + err.Error()); return false }
		state.file = reopened
	}
	...
	written, err := writeRecord(state.file, event)
	state.tailBytes = saturatingAdd(state.tailBytes, written)
	if err != nil {
	...
	case "clause":
		alternative := "none"
		if event.AlternativeIndex != nil { alternative = strconv.FormatUint(uint64(*event.AlternativeIndex), 10) }
		key := strconv.FormatUint(event.SwitchID, 10) + "\x00" + strconv.FormatUint(event.ClauseID, 10) + "\x00" + alternative + "\x00" + event.Event
	...
	if state.tailBytes >= state.compactAtTailBytes { compactLocked(state) }
	return true
}
...
func writeRecord(file *os.File, event record) (int64, error) {
	...
	if written != len(line) { return int64(written), fmt.Errorf("short write: wrote %d of %d bytes", written, len(line)) }
...
func compactLocked(state *writerState) {
	...
	defer func() {
		if !committed {
			state.compactAtTailBytes = compactionRetryThreshold(state.compactAtTailBytes, state.tailBytes)
		}
	}()
	...
	for _, key := range evaluationKeys {
		written, err := writeRecord(temporary, state.evaluations[key])
		snapshotBytes = saturatingAdd(snapshotBytes, written)
		if err != nil {
			_ = temporary.Close()
	...
	for _, event := range activeEvents {
		written, err := writeRecord(temporary, event)
		snapshotBytes = saturatingAdd(snapshotBytes, written)
		if err != nil {
			_ = temporary.Close()
	...
	for _, key := range clauseKeys {
		written, err := writeRecord(temporary, state.clauses[key])
		snapshotBytes = saturatingAdd(snapshotBytes, written)
		if err != nil {
			_ = temporary.Close()
	...
	committed = true
	state.tailBytes = 0
	state.compactAtTailBytes = compactTailThreshold(snapshotBytes)
...
func compactTailThreshold(snapshotBytes int64) int64 {
	if snapshotBytes > math.MaxInt64/compactSnapshotRatio {
		return math.MaxInt64
	}
	threshold := snapshotBytes * compactSnapshotRatio
	if threshold < minimumCompactTailBytes { return minimumCompactTailBytes }
	return threshold
}

func compactionRetryThreshold(current, tailBytes int64) int64 {
	doubled := saturatingAdd(current, current)
	afterMinimumTail := saturatingAdd(tailBytes, minimumCompactTailBytes)
	if doubled < afterMinimumTail { return afterMinimumTail }
	return doubled
}

func saturatingAdd(left, right int64) int64 {
	if right > math.MaxInt64-left { return math.MaxInt64 }
	return left + right
}
...
func diagnostic(packagePath, message string) {
	...
	fmt.Fprintln(os.Stderr, "gomcdc runtime diagnostic:", message)
}
