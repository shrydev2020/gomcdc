...
| primary solver backing arrays | 64 MiB | flatten済み式、memo、2 completion buffer、candidate index backing array |
...
solver backing arrayは`O(N + C + E)`であり、上表のbyte limitをallocation前に検査する。検証済み
input data、result witness、preprocessing、goroutine stack、その他のprocess memoryはこの単位に
含めない。この上限はprocess全体のheap、RSS、総memory上限ではない。
...
evaluation pair 1件、joint search state 47件、primary solver backing array 1,784 bytesで同じwitnessを
得る。
...
未観測数、covered/incomplete数、candidate pair、search state、solver byteを出力する。
...
C=48 caseのsolver-byte metricは746,776 bytesであり、`B/op`との差は主に92,670件の入力vectorを
検証・sortする入力比例のpreprocessingである。solver byte budgetは主要backing arrayを制限する
...
- evaluation pair、search state、solver byteのlimit直前とlimitちょうど
