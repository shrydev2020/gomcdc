...
案Aの実装・検証結果は`issues/runtime-journal-io-optimization-results.md`へ保存した。案A後に
残ったfull self measurementのtest、report、compiler toolchain preparationはjournal writer
と異なる責任なので、`issues/full-self-measurement-optimization-plan.md`へ分離する。このRJI
計画へreport/HTML/compiler cacheの変更を追加しない。
