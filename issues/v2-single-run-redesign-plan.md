...
`FileTransformation`はimmutable revision間のbyte来歴とfile名・物理位置・論理位置のmanifestだけを
担当し、coverage成立を決めない。
...
以下はv2のsingle-run cutoverとは独立した後続issueであり、v2 completionを遡及的に拡張しない。
各issueは記載した着手条件と完了条件を満たす場合だけ実装する。

### FOLLOW-001: 変換来歴modelのinternal化と抽出判定

Status: 2026-07-22完了。外部抽出は未実施であり、第二consumer出現時のgateを維持する。

gomcdcのAST rewrite、生成region manifest、`//line`処理、coverage correspondenceで得た知見を、
まずgomcdc内部の変換来歴modelとして整理する。modelはimmutableなsource revision間のhalf-open
byte range relationを扱い、元source由来、synthetic、deleted、ambiguous、lossyを区別する。
変換来歴はcoverage成立を決めず、Go cover固有のacceptanceとcorrespondenceは別境界に残す。

この段階で`goastmap`等の公開packageや独立repositoryを作らず、任意のAST変換が可逆であるとも
宣言しない。mutation testingなど第二の実利用が現れ、gomcdc向けに作ったAPIを用途固有の分岐や
曖昧なfallbackなしで自然に再利用できた時点で初めて抽出を検討する。抽出時は少なくとも次を
満たすことをgateとする。

- source revision identityと座標単位がAPI上で明示される。
- synthetic、deleted、one-to-many、many-to-one、lossy mappingの失敗意味が固定される。
- 中間revisionが一致する変換だけをcomposeできる。
- gomcdcと第二consumerの両方にsemantic differential testがある。
- 抽出後もgomcdcのcoverage correspondenceやfailure semanticsを汎用packageへ移さない。

実装結果:

- `internal/transformmap`にidentity、digest、size、byte座標を持つimmutable revisionと、全領域を
  partitionするhalf-open relationを実装した。preserved、synthetic、deleted、ambiguous、lossy、
  one-to-many、many-to-oneを区別する。
- composeは中間revisionのidentity、digest、sizeとrelation境界が完全一致する場合だけ成功し、
  partial alignmentは推測せず失敗する。
- 計装結果を`FileTransformation`へ統合した。byte-identical fileはpreserved、AST printerで
  再出力したsource由来byteはlossy、helper statementと計装専用`//line`はsyntheticと記録する。
- `CoverageCorrespondence`は独立したcoverage acceptance authorityのままとし、変換来歴から
  coverage成立を推測しない。

### FOLLOW-002: decision単位のMasking解析並行化

Status: 2026-07-22完了。代表workloadで逐次実装を上回ったためbounded並行実装を採用した。

複数decisionのMasking MC/DC analysisをbounded worker poolで並行化できるか再評価する。単一の重い
decision内の`jointSolver`はこのissueの対象外とする。着手前に複数decisionを持つ実workloadの
CPU profileを取り、逐次report構築がwall timeの支配要因であることを確認する。

実装する場合もcondition obligationごとのevaluation-pair、search-state、solver backing-array
budgetは変更しない。一方、同時実行数に応じてprocess全体のCPUとmemory使用量が増えるため、
worker数を明示的に制限し、per-obligation budgetをprocess総memory上限として扱わない。
unbounded goroutine、共有mutable solver workspace、入力順に依存するwitness選択は導入しない。

完了条件は、逐次版とsummary、status、witness、出力順序が一致し、`-race`、cancellation、panic、
partial evidence、goroutine leakのtestを通り、代表的な複数decision benchmarkでwall time改善を
再現できることである。改善が確認できなければ逐次実装を維持する。

実装・検証結果:

- 着手前profileでは24 decision・各8 conditionのMasking report構築が約4.03 ms/opで、
  `report.Build`がCPU sampleの43.54%、`buildDecisionReport`が38.69%、Masking解析が17.07%を占め、
  decision report構築がapplication側の支配経路であることを確認した。
- Masking選択時だけdecision単位で並行化し、実効worker数を
  `min(decision数, GOMAXPROCS, 4)`に制限した。solver workspaceと既存のcondition obligation単位
  budgetは共有も変更もしない。
- sort済みtask indexへ結果を書き戻してから逐次集約するため、worker数にかかわらずsummary、
  status、witness、出力順が一致する。cancellationは未開始Maskingだけを
  `analysis-incomplete`とし、受理済みevidenceとUnique-Causeを保持する。panic時は全workerの終了後に
  caller goroutineで再panicする。
- 同一benchmarkの3回中央値は4,031,632 ns/opから2,388,727 ns/opへ40.75%短縮（1.69倍）した。
  allocationは5,922,681 B/opから5,966,309 B/opへ0.74%、32,320 allocs/opから32,347へ0.08%
  増加した。最終codeでの明示的な1 worker対4 workerの5回中央値は4.206 ms/op対2.415 ms/opで
  42.6%短縮した。
- 決定性、cancellation、partial evidence、panic、worker cap、goroutine leakをtest化し、raceを
  completion gateに含める。

### FOLLOW-003: stable Go 1.27対応

Go 1.27のstable release後に、Go 1.26.xを最低対応versionとして維持したまま、選択toolchainとして
Go 1.27.xを追加する。単一の`go1.26`定数を`go1.27`へ置き換えて旧seriesを捨てず、stable seriesを
明示的に列挙して未知versionをfail closedにする。RCや`devel`をstable対応として暗黙受理しない。

2026-07-22時点のGo 1.27rc2ではcompiler-aware patchの11 exact anchorが各一度存在することを
確認したが、これはstable互換性の証明ではない。正式対応では次を完了条件とする。

- stable Go 1.27のcompiler sourceへexact-anchor patchを適用し、patched compilerをbuildできる。
- generic methodを含むsource、相対`//line`、generated helper、Go cover correspondenceをfixtureで
  検証する。
- `golang.org/x/tools`をGo 1.26と1.27の両方で必要なversionへ更新する。
- CIをGo 1.26.xと1.27.xのmatrixにし、test、race、vet、strict self-MC/DC baselineを通す。
- `go.mod`の`go 1.26.0`は、Go 1.27専用APIが実装上不可欠でない限り引き上げない。
- READMEと日英仕様へ対応toolchainとGo 1.27のplatform制約を反映する。
