...
小規模・早期成立の経路を残している。Maskingもevaluation×conditionごとの
completion/pivotal判定をpair探索前に一度だけ計算するよう変更した。8条件・256評価の
completion benchmarkは `1.67 ms/op`（2.3 MB/op）で、pairごとの式木再走査を除去した。
...
`150,205 allocs/op` だった。これはcollectorの測定であり、生成runtimeのwriter lockを
直接測った値ではない。writerを非同期化・分割する変更は、flush、panic、Goexit、異常終了
時の耐久性を同時に証明できるfixtureなしでは採用しない。
