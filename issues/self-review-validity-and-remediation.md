...
リポジトリ直下に `LICENSE`、`COPYING`、または同等のプロジェクトライセンス
ファイルはない。`THIRD_PARTY_NOTICES.md` にあるのは、`internal/c0` が利用する
Go `cmd/cover`由来コードのBSD系通知だけであり、本リポジトリ自身のライセンス
ではない。

したがって、現時点で利用者へ再配布・改変権を宣言できない。ライセンスを勝手に
選択して追加してはいけないため、著作権者の選択が必要である。

### L-001 プロジェクトライセンスを決定する

- **状態:** 要決定
- **対応:** 著作権者がライセンス（例: BSD-3-Clause、MIT、Apache-2.0等）を選択する。
  選択後にその全文を `LICENSE` として追加し、READMEのライセンス節を実際の内容へ
  更新する。
- **完了条件:** `LICENSE` が存在し、README、配布物、third-party noticeの関係が
  一貫している。ライセンスを選ぶまでは「未定義」と記述する。
...
**現状:** `mcdc.UniqueCauseStrategy` と `MaskingStrategy` は、target conditionごとに
評価pairを二重ループする。Unique-Causeは `O(C × E²)`。Maskingはpairごとに
`pivotalCompletion` 等で式木を走査するため、`O(C × E² × S)` を上限として扱う。
...
3. Maskingはevaluationごとのcompletion/pivotal情報をtarget単位で先に計算し、pair
   ループで式木を再評価しない。masked conditionの証跡は保持する。
...
**現状:** `internal/runtimecov` はprocess内の共有mapを `recordMu` で保護し、同じmutexを
保持したままJSONL writeとcompactionを行う。Conditionそのものは各evaluationの
memory stateへ書くだけだが、Begin/terminal/clause eventは同期I/O経路に入る。
...
**完了条件:** goroutine/package数を増やしたbenchmarkで競合を確認し、runtime eventが
失われない。速度改善がなければ変更を採用しない。
...
1. **L-001:** プロジェクトライセンスの決定（コード変更の前提）。
